...
class Hom_sales_target_prediction extends ROOT_Controller
{
    private  $message;
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->load->model("hom_sales_target_prediction_model");
    }
    ...
    public function budget_add_edit()
    {
        ...
        $data['title']="HOM Sales Target Prediction";
        $ajax['page_url']=base_url()."hom_sales_target_prediction/index/add";

        $ajax['status']=true;
        $ajax['content'][]=array("id"=>"#content","html"=>$this->load->view("hom_sales_target_prediction/add_edit",$data,true));
                ...
                foreach($predictionTypeVariety as $type_id=>$predictionVariety)
                {
                    foreach($predictionVariety as $variety_id=>$YearDetail)
                    {
                        foreach($YearDetail as $year=>$quantity)
                        {
                            ...
                            if($quantity>0)
                            {
                                $prediction_existing_id = $this->hom_sales_target_prediction_model->get_hom_sales_target_existence($year, $this->input->post('year'), $variety_id);
    ...
    public function get_variety_detail()
    {
        ...
        $data['prediction_years'] = $this->hom_sales_target_prediction_model->get_prediction_years($year);
        ...
        $data['varieties'] = $this->hom_sales_target_prediction_model->get_varieties_by_crop_type($crop_id, $type_id);

        if(isset($data['year']) && sizeof($data['varieties'])>0)
        {
            $ajax['status'] = true;
            $ajax['content'][]=array("id"=>'#variety_quantity',"html"=>$this->load->view("hom_sales_target_prediction/variety",$data,true));
            $this->jsonReturn($ajax);
        }
        else
        {
