...
class Pricing_marketing extends ROOT_Controller
{
    private  $message;
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->load->model("pricing_marketing_model");
    }
    ...
    public function budget_add_edit($id)
    {
        ...
        if(strlen($id)>1)
        {
            $data['quantity_setups'] = $this->pricing_marketing_model->get_confirmed_quantity_detail($id);
            $data['title'] = "Edit Pricing Marketing";
            $ajax['page_url']=base_url()."pricing_marketing/index/edit/";
        }
        else
        {
            $data['quantity_setups'] = array();
            $data['title'] = "Pricing Marketing";
            $ajax['page_url'] = base_url()."pricing_marketing/index/add";
        }

        $ajax['status'] = true;
        $ajax['content'][] = array("id"=>"#content","html"=>$this->load->view("pricing_marketing/add_edit",$data,true));
    ...
    public function budget_save()
    {
        ...
        $data['pricing_type'] = $this->config->item('pricing_type_marketing');
            ...
            foreach($pricingPost as $crop_id=>$typeVarietyPost)
            {
                ...
                foreach($typeVarietyPost as $type_id=>$varietyPost)
                {
                    ...
                    foreach($varietyPost as $variety_id=>$detailPost)
                    {
                        ...
                        if($data['mrp']>0)
                        {
                            $edit_id = $this->pricing_marketing_model->check_sales_pricing_existence($year, $crop_id, $type_id, $variety_id);
    ...
    public function get_quantity_detail_by_variety()
    {
        ...
        $data['varieties'] = $this->pricing_marketing_model->get_varieties_by_crop_type($crop_id, $type_id);

        if(isset($data['year']) && sizeof($data['varieties'])>0)
        {
            $data['title'] = 'Pricing Marketing';
            $ajax['status'] = true;
            $ajax['content'][] = array("id"=>'#variety_quantity',"html"=>$this->load->view("pricing_marketing/variety_list",$data,true));
            $this->jsonReturn($ajax);
        }
        else
        {
...
//    {
//        $year = $this->input->post('year');
//        $existence = $this->pricing_marketing_model->check_quantity_year_existence($year);
//
//        if($existence)
//        {
//            $ajax['status'] = false;
