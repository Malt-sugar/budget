...
class Approval_sales_target_di extends ROOT_Controller
{
    private  $message;
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->load->model("approval_sales_target_di_model");
    }
    ...
    {
        $config = System_helper::pagination_config(base_url() . "approval_sales_target_di/index/list/",$this->approval_sales_target_di_model->get_total_customers(),4);
        ...
        $data['sales_targets'] = $this->approval_sales_target_di_model->get_sales_target_info($page);
        $data['title']="DI Sales Target Approval";

        $ajax['status']=true;
        $ajax['content'][]=array("id"=>"#content","html"=>$this->load->view("approval_sales_target_di/list",$data,true));
        if($this->message)
        {
            $ajax['message']=$this->message;
        }
        $ajax['page_url']=base_url()."approval_sales_target_di/index/list/".($page+1);
    ...
    public function budget_add_edit($customer_id, $year_id)
    {
        ...
        {
            $data['targets'] = $this->approval_sales_target_di_model->get_sales_target_detail($customer_id, $year_id);
            $data['title'] = "DI Sales Target Approval";
            $ajax['page_url']=base_url()."approval_sales_target_di/index/edit/".$customer_id.'/'.$year_id;
        }
        else
        {
            $data['targets'] = array();
            $data['title'] = "Sales Target Approval";
            $ajax['page_url'] = base_url()."approval_sales_target_di/index/add";
        }

        $ajax['status'] = true;
        $ajax['content'][] = array("id"=>"#content","html"=>$this->load->view("approval_sales_target_di/add_edit",$data,true));
    ...
    public function budget_save()
    {
        ...
        else
        {
            ...
            if(strlen($this->input->post('customer_id'))>1 && strlen($this->input->post('year_id'))>1)
            {
                ...
                $existing_record_varieties = $this->approval_sales_target_di_model->get_existing_sales_targets_records($this->input->post('customer_id'), $this->input->post('year_id'));
                ...
                $existing_varieties = $this->approval_sales_target_di_model->get_existing_sales_targets($this->input->post('customer_id'), $this->input->post('year_id'));
                        ...
                        if($quantityKey==$cropTypeKey)
                        {
                            ...
                            foreach($quantity as $variety_id=>$amount)
                            {
                                ...
                                if(in_array($variety_id, $existing_varieties))
                                {
                                    ...
                                    $data['status'] = 1;
                                    $data['is_approved_by_di'] = 1; // ZI Approval
    ...
    public function get_varieties_by_crop_type()
    {
        ...
        $data['varieties'] = $this->approval_sales_target_di_model->get_variety_by_crop_type($crop_id, $type_id);
        ...
        if(sizeof($data['varieties'])>0)
        {
            ...
            $ajax['status'] = true;
            $ajax['content'][]=array("id"=>'#variety'.$current_id,"html"=>$this->load->view("approval_sales_target_di/variety_list",$data,true));
            $this->jsonReturn($ajax);
        }
        else
        {
