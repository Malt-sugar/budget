...
class Di_sales_target extends ROOT_Controller
{
                    ...
                    foreach($varietyDetail as $type_id=>$varietyInfo)
                    {
                        ...
                        foreach($varietyInfo as $variety_id=>$detail)
                        {
                            ...
                            if($data['budgeted_quantity']>0)
                            {
                                ...
                                $notification_id = $this->di_sales_target_model->check_notification_existence($year, $variety_id);
                                if($notification_id)
                                {
                                ...
                                if($this->input->post('forward') && $this->input->post('forward')==1)
                                {
                                    ...
                                    $nid = $this->di_sales_target_model->check_notification_existence_for_hom($year, $variety_id);
                                    if($nid)
                                    {
    ...
    public function get_variety_detail()
    {
        ...
        $data['varieties'] = $this->di_sales_target_model->get_varieties_by_crop_type($crop_id, $type_id);

        if(isset($data['year']) && sizeof($data['varieties'])>0)
        {
            $ajax['status'] = true;
            $ajax['content'][]=array("id"=>'#variety_quantity',"html"=>$this->load->view("di_sales_target/variety",$data,true));
            $this->jsonReturn($ajax);
        }
        else
        {
