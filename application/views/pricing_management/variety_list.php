    ...
    <div class="col-lg-12 main_div" style="overflow: auto;">
        <table class="table table-hover table-bordered">
            ...
            foreach($varieties as $key=>$variety)
            {
                ...
                {
                    $net_sales_price = round($existing_data['mrp'] - ($existing_data['sales_bonus']/100)*$existing_data['mrp'] - ($existing_data['other_incentive']/100)*$existing_data['mrp'] - ($detail['sales_commission']/100)*$existing_data['mrp']);
                    $total_net_sales_price = round($net_sales_price*$detail['targeted_quantity']);
                    $grand_total_net_sales += $total_net_sales_price;
                    $net_profit = round($net_sales_price - $total_cogs);
            ...
            <tr class="main_tr">
                ...
                <td class="text-center">
                    <?php echo $variety['varriety_name'];?>
                    <input type="hidden" name="total_cogs" class="total_cogs" value="<?php echo $total_cogs;?>" />
                    <input type="hidden" name="cogs" class="cogs" value="<?php echo $detail['cogs'];?>" />
                    <input type="hidden" name="targeted_quantity" class="targeted_quantity" value="<?php echo $detail['targeted_quantity'];?>" />
                </td>
                ...
                <td class="text-center"><input type="text" name="pricing[<?php echo $variety['crop_id'];?>][<?php echo $variety['product_type_id'];?>][<?php echo $variety['varriety_id'];?>][mrp]" class="form-control mrp_by_mgt numbersOnly trigger_class" value="<?php echo isset($existing_data['mrp'])?$existing_data['mrp']:'';?>" /></td>
                <td class="text-center"><input type="text" name="pricing[<?php echo $variety['crop_id'];?>][<?php echo $variety['product_type_id'];?>][<?php echo $variety['varriety_id'];?>][sales_commission]" class="form-control sales_commission numbersOnly trigger_class" value="<?php if(isset($existing_data['sales_commission'])){echo $existing_data['sales_commission'];}else{echo $detail['sales_commission'];}?>" /></td>
            ...
            <tr class="grand_tr">
                ...
                <td class="text-center" style="vertical-align: middle;"><label class="label label-danger grand_total_net_sales"><?php echo $grand_total_net_sales;?></label></td>
                <td class="text-center" style="vertical-align: middle;"><label class="label label-danger grand_total_net_profit"><?php echo $grand_total_net_profit;?></label></td>
                <td class="text-center"></td>
                <td class="text-center"></td>
            </tr>
        </table>
...
<script>
    ...
    jQuery(document).ready(function()
    {
        ...
        $(document).on("keyup",".trigger_class",function()
        {
            ...
            var mrp_by_mgt = parseFloat($(this).closest('.main_tr').find(".mrp_by_mgt").val());

            var net_sales_price = Math.round((mrp_by_mgt - (sales_commission/100)*mrp_by_mgt - (sales_bonus/100)*mrp_by_mgt - (other_incentive/100)*mrp_by_mgt));
        ...
        {
            var mrp = parseFloat($(this).closest('.main_tr').find('.mrp_by_mgt').val());
            var sales_commission = parseFloat($(this).val());
            var sales_commission_amount = Math.round(((sales_commission/100)*mrp));
        ...
        $(document).on("keyup",".sales_bonus",function()
        {
            ...
            var sales_bonus_amount = Math.round(((sales_bonus/100)*mrp));
        ...
        $(document).on("keyup",".other_incentive",function()
        {
            ...
            var other_incentive_amount = Math.round(((other_incentive/100)*mrp));
