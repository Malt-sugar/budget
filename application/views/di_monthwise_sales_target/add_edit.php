<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    $data["link_new"]=base_url()."di_monthwise_sales_target/index/add";
    $data["link_back"]="#";
    $data["link_approve"]="#";
    $data["hide_approve"]="1";
    $data["link_save"]="#";
...
<form class="form_valid" id="save_form" action="<?php echo base_url();?>di_monthwise_sales_target/index/save" method="post">
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <select name="type" id="type" class="form-control validate[required]">
                    ...
                    <option value="1"><?php echo $this->lang->line('ZONEWISE');?></option>
                    <option value="2"><?php echo $this->lang->line('DIVISIONWISE');?></option>
                </select>
            </div>
        </div>

        <div class="row show-grid" id="zone_div" style="display: none;">
            <div class="col-xs-4">
        ...
        $(document).on("change","#type",function()
        {
            ...
            else
            {
                ...
                if($(this).val()==2 && $("#year").val().length>0)
                {
                    $("#load_variety").show();
                    $.ajax({
                        url: base_url+"di_monthwise_sales_target/get_variety_detail/",
                        type: 'POST',
                        dataType: "JSON",
        ...
        $(document).on("change","#zone",function()
        {
            if($("#type").val()==1 && $("#year").val().length>0 && $(this).val().length>0)
            {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: "JSON",
                    data:{year_id:$("#year").val(), type: $("#type").val(), zone: $(this).val()},
                    success: function (data, status)
                    {
