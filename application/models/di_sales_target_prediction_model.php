...
class Di_sales_target_prediction_model extends CI_Model
{
    ...
    public function get_division_sales_prediction($year, $variety)
    {
        $user = User_helper::get_user();
        ...
        $this->db->where('bstp.division_id', $user_division);
        $this->db->where('length(bstp.zone_id)>2');
    ...
    public function get_di_sales_target_existence($year, $prediction_year, $variety)
    {
        $user = User_helper::get_user();
        $user_division = $user->division_id;
        $this->db->from('budget_sales_target_prediction bstp');
